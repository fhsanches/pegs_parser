...
;; a struct containing the results of a successful parse
...
(defun with-peg-parse (peg-name text-input)
  "Reads a file ./pegs/peg-name and parses it, printing the results of the parse"
...
;; Starting from here, each function will define a parser for a rule with its respective
;; name in the PEGs paper by Bryan Ford (2004)
...
(defun seq (funlist &optional pos)
  "Parses a list 'f1 f2 ... fn'. Fails if any f parser fails"
  (let*
      ((init-pos *pos*)
...
(defun many-star (funform)
  (let ((this (try funform)))
    ...
    (let ((next (many-star funform)))
      (cond
        ((eql (car next) t) ;; next retuned 't', end star
         (cons this '()))
        (next ;; next exists and is not t
         (cons this next))))
    (cons t '())))) ;; I do not exist, return (t) (t in a list!)
...
;; list the rules of a grammar (hash)
